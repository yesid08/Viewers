...
function n8Array(y, x, pixelValue, shape_img){
    const W = shape_img[1]
    const array_values = [pixelValue[x + 1 + (y + 1) * W], pixelValue[x + 1 + y * W], pixelValue[x + 1 + (y - 1) * W], pixelValue[x + (y - 1) * W], pixelValue[x - 1 + (y - 1) * W], pixelValue[x - 1 + y * W], pixelValue[x - 1 + (y + 1) * W], pixelValue[x + (y + 1) * W], pixelValue[x + 1 + (y + 1) * W]]
    return array_values
}
...
function n8UpDown(idx, active_label, pixelValue, shape_img){ 
    ...
    const bordersDown = [pixelValue[x + 1 + (y + 1) * W] , pixelValue[x - 1 + (y + 1) * W] , pixelValue[x + (y + 1) * W]]
    const bordersUp = [pixelValue[x + 1 + (y - 1) * W] , pixelValue[x + (y - 1) * W] , pixelValue[x - 1 + (y - 1) * W]]
    return [countSameNeighbor(bordersDown,active_label).reduce(sumArray), countSameNeighbor(bordersUp,active_label).reduce(sumArray)]
}
    ...
    for (const sliceData of codedAnnotData.slicesData) {
        ...
        for (const segmentData of sliceData.segmentsData) {
            ...
            for (const direction of segmentData.code) {
                if ([2, 3, 4].includes(direction)) { y -= 1 }
                if ([6, 7, 8, 0].includes(direction)) { y += 1 }
                if ([4, 5, 6].includes(direction)) { x -= 1 }
                if ([2, 1, 0, 8].includes(direction)) { x += 1 }
            ...
            for (const coords of coords_countour){
                ...
                if (sum_array <= thresh){
                    if (twoConsecutiveN8(n8.slice(0,n8.length), segmentData.label) || sum_array <= segmentData.label){
            ...
            for (let y_coord = minY; y_coord < maxY; y_coord++) {
                ...
                for (let x_coord = minX; x_coord < maxX; x_coord++) {
                    ...
                    if (pixelOrigBorder[idx] && pixelOrigBorder[idx] != pixelOrigBorder[idx-1] && !intoBorder){
                        let resultN8UpDownIn = n8UpDown(idx, current_label, pixelOrigBorder, shape_img)
                        upIn = resultN8UpDownIn[0]
                        downIn = resultN8UpDownIn[1]
                        if (!(Boolean(upIn) && Boolean(downIn))){ intoBorder=true}
                    }
                    else if (current_label != prev_label && intoBorder){
                        intoBorder=false
                        let resultN8UpDownOut = n8UpDown(idx, prev_label, pixelOrigBorder, shape_img)
                        upOut = resultN8UpDownOut[0]
                        downOut = resultN8UpDownOut[1]
                        if ((Boolean(upIn) == Boolean(upOut)) && (Boolean(downIn) == Boolean(downOut))){
                            if (arrayIn.length){ arrayIn.shift()}
                            else { arrayIn.push(prev_label)}
                            if (arrayIn.length){
                    ...
                    if (pixelOrigBorder[idx] == pixelOrigBorder[idx-1] && arrayIn.includes(current_label)){ continue }
                    if ((current_label && !prev_label) || (current_label != prev_label && current_label && prev_label)){
                    ...
                    else if (current_label && prev_label && current_label == prev_label && arrayIn.includes(current_label)){
                        arrayIn.shift()
                    }
                }
            }
        ...
        for (let active_label of dataSlice.segmentsOnLabelmap){
            ...
            for (let layerInfo of layers){
                ...
                if (layer_label == active_label){
                    for (let idx = 0; idx < layer_per_label.length; idx++){
                        layer_per_label[idx] = Boolean(layer_per_label[idx]) != Boolean(layerInfo[0][idx])
                        layer_per_label[idx] = Boolean(layer_per_label[idx]) || Boolean(layers_borders[idx_layer][idx])
                    }
                }
                idx_layer++
            }
            for (let idx = 0; idx < layer_per_label.length; idx++){
                ...
                dataSlice.pixelData[idx] = dataSlice.pixelData[idx] + layer_per_label[idx]
            }
        }
