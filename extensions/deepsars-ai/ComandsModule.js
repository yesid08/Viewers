            ...
            promisePetition
              .then(response => {
                ...
                if (response.hasOwnProperty('error')) {
                  //Handle model with less than 20 slices
                  UINotificationService.show({
                    title: 'Error de Predicción Covid',
                    type: 'warning',
                    duration: 5 * 1000,
                    position: 'topRight',
                    message: 'Por favor intente de nuevo',
                  ...
                  if (parseFloat(response.Covid) >= parseFloat(response.No-covid)) {
                    predictions.predictMultiplePathologies(
                      ...
                      if (res.hasOwnProperty('error')) {
                        //Handle model with less than 20 slices
                        UINotificationService.show({
                          title: 'Error de Predicción Sars',
                          type: 'warning',
                          duration: 5 * 1000,
                          position: 'topRight',
                          message: 'Por favor intente de nuevo',
                        ...
                        response.Sars = res.SARS;
                        utils.generate_distribution('Probabilidades por clase',
                          '',
                          response,
                          services,
                          MODELS_INFORMATION.ct_2d.train_samples,
                          MODELS_INFORMATION.ct_2d.acc);
                      }
                    });
                  } else {
                    UINotificationService.show({
                      title: 'Resultado:',
                      type: 'success',
                      message: `Se estima una probabilidad de ${response.No-covid}% No-covid`,
                      duration: 1000 * 5,
                    });
      ...
      predictAxialMultimodalSliceCt: {
        commandFn: () => {
          if (utils.isSeriesCT()) {
            ...
            const payloadData = {
              microservice: 'orthanc',
              task: 'multimodal_prediction',
              file_ID: dicomData.SOPInstanceUID,
              file_type: 'slice',
              file_mod: 'ct',
              file_view: 'axial',
              task_class: 'classify',
              task_mode: 'multimodal_covid'
            };
